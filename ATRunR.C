...
//th0 should have a negative sign to be consistent with the phi definitions
//adc_up - upstream detector
//adc_lo - downstream detector
//Ebeam in GeV - Run dependent...
void ATRunR(int run, double th0, double adc_up, double adc_lo, double Ebeam, TString targ){ 
...
//   T->Add(Form("/lustre19/expphy/volatile/halla/parity/crex_optics_rootfiles/prexRHRS_%d_-1*.root",run));
   T->Add(Form("../prex2files/prexRHRS_%d_-1*.root",run));
   ...
   for(int i = 0; i < 2; i++){ 
      ...
      Qsq[i] = new TH1F(Form("Qsq[%i]",i),Form("RHRS Q^{2} for %s Main Detector, Run %d, %s",det[i],run,targ.Data()),150,0,0.015);//0.07 for CREX
      //Relative to horizontal 
      Phi[i] = new TH1F(Form("Phi[%i]",i),Form("RHRS #Phi_{H} for %s Main Detector, Run %d, %s",det[i],run, targ.Data()),150,135,225);
     //Relative to vertical
     Cos[i] = new TH1F(Form("Cos[%i]",i),Form("RHRS Cos(#Phi_{V}) for %s Main Detector, Run %d, %s",det[i],run, targ.Data()),150,-0.8,0.8);
     Sin[i] = new TH1F(Form("Sin[%i]",i),Form("RHRS Sin(#Phi_{V}) for %s Main Detector, Run %d, %s",det[i],run, targ.Data()),150,0.6,1.05);
    ...
    T->SetBranchAddress("P.loQadcR",&thisloADC);
...
//   c_ph->SaveAs(Form("phiRHRS_Run%d.pdf",run));
   ...
   ctest->cd(4);
   Phi[0]->Draw("HIST");
   ctest->SaveAs(Form("KineRHRSupdet_Run%d_%s.pdf",run,targ.Data()));
   ...
   ctest1->cd(4);
   Phi[1]->Draw("HIST");
   ctest1->SaveAs(Form("KineRHRSdowndet_Run%d_%s.pdf",run,targ.Data())); 
