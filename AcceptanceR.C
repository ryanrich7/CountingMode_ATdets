...
//Ebeam in GeV
void AcceptanceR(int run, double th0, double adc_up, double adc_in, double adc_out, double Ebeam){ 
   ...
   for(int i = 0; i < 3; i++){ 
      ...
      Qsq[i] = new TH1F(Form("Qsq[%i]",i),Form("RHRS Q^{2} for %s Detector, Run %d",det[i],run),150,0,0.035);
     ...
     Sin[i] = new TH1F(Form("Sin[%i]",i),Form("RHRS Sin(#Ph_{V}) for %s Detector, Run %d",det[i],run),150,0.6,1.05);
     //Thtg and Phtg
...
//    double thisoutCosAng,thisoutADC,thisoutQsq,thisoutCosPhi,thisoutSinPhi,thisoutPhi;
  ...
  for(int i = 0; i < 3; i++){
   ...
   Qsq[i]->Draw("HIST");
   c_q[i]->SaveAs(Form("qsqRHRS_%s_Run%d.pdf",det[i],run));


   c_cos[i] = new TCanvas(Form("c_cos[%i]",i),Form("c_cos[%i]",i));
   Cos[i]->Draw("HIST");
   c_cos[i]->SaveAs(Form("cosRHRS_%s_Run%d.pdf",det[i],run));
  
   c_sin[i] = new TCanvas(Form("c_sin[%i]",i),Form("c_sin[%i]",i));
   Sin[i]->Draw("HIST");
   c_sin[i]->SaveAs(Form("sinRHRS_%s_Run%d.pdf",det[i],run));

   c_ph[i] = new TCanvas(Form("c_ph[%i]",i),Form("c_ph[%i]",i));
   Phi[i]->Draw("HIST");
   c_ph[i]->SaveAs(Form("phiRHRS_%s_Run%d.pdf",det[i],run));

   c_thtg[i] = new TCanvas(Form("c_thtg[%i]",i),Form("c_thtg[%i]",i));
   ThTg[i]->Draw("HIST");
   c_thtg[i]->SaveAs(Form("thtgRHRS_%s_Run%d.pdf",det[i],run));

   c_phtg[i] = new TCanvas(Form("c_phtg[%i]",i),Form("c_phtg[%i]",i));
   PhTg[i]->Draw("HIST");
   c_phtg[i]->SaveAs(Form("phtgRHRS_%s_Run%d.pdf",det[i],run));
